...
test "reference operations - ref ref" {
    ...
    try expectEqual(100, tensor.scalar(.{ 0, 0 }));
    try expectEqual(200, tensor.scalar(.{ 0, 1 }));
    try expectEqual(300, tensor.scalar(.{ 1, 0 }));
    try expectEqual(400, tensor.scalar(.{ 1, 1 }));
}
...
test "reference operations - wiseNew through reference" {
    ...
    const result = try op.wise(std.testing.allocator, .{ tensor1_ref, tensor2_ref }, func.addFactory(f64, 2));
