...
test "reference operations - ref ref" {
    ...
    try expectEqual(100, tensor.constRef(.{0}));
    try expectEqual(200, tensor.constRef(.{1}));
    try expectEqual(300, tensor.constRef(.{2}));
}
...
test "reference operations - wiseNew through reference" {
    ...
    const result = try op.wise(std.testing.allocator, .{ tensor1_ref, tensor2_ref }, (struct {
