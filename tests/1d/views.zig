...
test "ref operations - reshape" {
    ...
    var tensor = Tensor(f64, 1).from(.rowMajor(.{6}), data[0..]);
    var ref = tensor.ref(.{});
    const reshaped = ref.reshape(.{ 2, 3 });

    try expectEqual(.{ 2, 3 }, reshaped.metadata.shape);
...
test "ref operations - slice" {
    ...
    try expectEqual(.{3}, sliced.metadata.shape);
...
test "Tensor operations - all const operations" {
    var data: [3]f64 = createSequence(f64, 3);
    var ref = Tensor(f64, 1).from(.rowMajor(.{3}), data[0..]);
    ...
    try expectEqual(.{ 1, 3 }, reshaped.metadata.shape);
    try expectEqual(data[0], reshaped.scalar(.{ 0, 0 }));
    try expectEqual(data[1], reshaped.scalar(.{ 0, 1 }));
    try expectEqual(data[2], reshaped.scalar(.{ 0, 2 }));
}
